...
drop if _merge==2
xtset groupid year
gen byte anal_smpl=robustvar==1 & not_yet_importer==1 & cityid!=. & streetid!=. & buildingid!=. & year>=1994 & L.cityid!=. & L.streetid!=. & L.buildingid!=.
preserve
...
quietly {
    log using "`out'/additional_descriptives.txt", text append
	noisily display "Number of firm-country-year observations in the estimation sample"
	noisily distinct tax_id if anal_smpl==1 
	log close
}
preserve
